...
window.addEventListener('message', event => {
	if (event.origin === "https://embed.mystream.to") {
		return;
	}
...
function poll_activity() {
    ...
    if (document.getElementById("videoPlayer_html5_api") == null) {
        return;
    }

    var currentTime = Math.floor(document.getElementById("videoPlayer_html5_api").currentTime),
        duration =  Math.floor(document.getElementById("videoPlayer_html5_api").duration);

    if (isNaN(currentTime) || isNaN(duration)) {
        return;
    }

    var playing = !document.getElementById("videoPlayer_html5_api").paused;
